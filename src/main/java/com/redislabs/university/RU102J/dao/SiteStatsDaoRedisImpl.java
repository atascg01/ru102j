...
public class SiteStatsDaoRedisImpl implements SiteStatsDao {
    ...
    private void updateOptimized(Jedis jedis, String key, MeterReading reading) {
        ...
        CompareAndUpdateScript compareAndUpdateScript1 = new CompareAndUpdateScript(jedisPool);
        compareAndUpdateScript1.updateIfGreater(transaction, key, SiteStats.maxWhField, reading.getWhGenerated());
        compareAndUpdateScript1.updateIfGreater(transaction, key, SiteStats.maxCapacityField, getCurrentCapacity(reading));
        compareAndUpdateScript1.updateIfLess(transaction, key, SiteStats.minWhField, reading.getWhGenerated());
        transaction.exec();
        // END Challenge #3
    }
