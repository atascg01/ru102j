...
public class SiteGeoDaoRedisImpl implements SiteGeoDao {
    ...
    public Set<Site> findAll() {
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            Set<Response<Map<String, String>>> responseSites = new HashSet<>(keys.size());
            ...
            for (Response<Map<String, String>> site : responseSites) {
                Map<String, String> siteMap = site.get();
                if (!siteMap.isEmpty()) {
    ...
    private Set<Site> findSitesByGeoWithCapacity(GeoQuery query) {
        ...
        GeoUnit radiusUnit = query.getRadiusUnit();
         ...
         try (Jedis jedis = jedisPool.getResource()) {
             ...
             List<GeoRadiusResponse> radiusResponses = jedis.georadius(key, coord.getLng(),
             ...
             for (Site site : sites) {
                 ...
                 scores.put(siteId, pipeline.zscore(capacityKey, String.valueOf(siteId)));
             }
             pipeline.sync();
             // END Challenge #5
